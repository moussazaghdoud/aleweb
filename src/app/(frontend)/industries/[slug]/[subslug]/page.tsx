            ...
            <div className="grid sm:grid-cols-2 gap-8">
              {subPage.benefitCards.map((card, ci) => (
                <div key={card.audience} className="rounded-2xl overflow-hidden shadow-lg">
                  <div className={`px-7 py-5 ${ci === 0 ? "bg-gradient-to-r from-slate-800 to-indigo-900" : "bg-gradient-to-r from-indigo-800 to-purple-900"}`}>
                    ...
                    <ul className="space-y-4">
                      {card.benefits.map((benefit, j) => (
                        <li key={j} className="flex items-start gap-3">
                          <div className={`w-5 h-5 rounded-full ${ci === 0 ? "bg-indigo-100" : "bg-purple-100"} flex items-center justify-center shrink-0 mt-0.5`}>
                            <svg className={`w-3 h-3 ${ci === 0 ? "text-indigo-600" : "text-purple-600"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                              <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                            </svg>
                          </div>
