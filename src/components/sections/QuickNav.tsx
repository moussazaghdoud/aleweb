...
const pillars = [
  {
    ...
    image: "/images/pillars/network1.jpg",
  },
  {
    id: "cloud" as const,
    ...
    image: "/images/pillars/cloud1.jpg",
  },
  {
    id: "ai" as const,
...
function IconSupport({ className = "w-6 h-6" }: { className?: string }) {
  return (
    <svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M11.42 15.17l-5.384 3.164a1 1 0 01-1.452-1.053l.696-5.955L.874 6.838a1 1 0 01.554-1.706l5.95-.514 2.81-5.387a1 1 0 011.796 0l2.81 5.387 5.95.514a1 1 0 01.554 1.706l-4.404 4.488.696 5.955a1 1 0 01-1.452 1.053L11.42 15.17z" />
    </svg>
  );
}
function IconTraining({ className = "w-6 h-6" }: { className?: string }) {
  return (
    <svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
function IconManaged({ className = "w-6 h-6" }: { className?: string }) {
  return (
    <svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
function IconSuccess({ className = "w-6 h-6" }: { className?: string }) {
  return (
    <svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
function IconIndustryServices({ className = "w-6 h-6" }: { className?: string }) {
  return (
    <svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
const serviceIcons = [IconSupport, IconTraining, IconManaged, IconSuccess, IconIndustryServices];

/* ── Featured case studies (diverse industries) ── */
const featuredCaseSlugs = [
  "korea-university-medicine",
  "south-palms-resort-and-spa-panglao",
  "javeriana-university",
  "transit-wireless",
  "strasbourg-eurometropolis",
  "dilo",
];
const featuredCaseStudies = featuredCaseSlugs
  .map((s) => caseStudiesData.find((c) => c.slug === s))
...
export function QuickNav() {
  return (
    <>
      ...
      <section className="pt-48 pb-20 bg-gradient-to-b from-gray-950 via-gray-900 to-white relative overflow-hidden">
        {/* Subtle glow blobs */}
                ...
                <FadeIn key={p.id} delay={i * 150}>
                  ...
                  >
                    ...
                    <div className="relative h-36 overflow-hidden rounded-t-2xl">
                      ...
                      <div className="absolute bottom-0 left-0 right-0 h-10 bg-gradient-to-b from-transparent to-gray-100" />
                    </div>

                    {/* Content */}
                    <div className="relative px-7 pt-4 pb-0 flex-1">
                      {/* Number + label */}
                      <div className="flex items-center gap-3 mb-4">
                        <span className={`w-8 h-8 rounded-lg ${c.numberBg} flex items-center justify-center text-white text-sm font-bold`}>
                          {String(i + 1).padStart(2, "0")}
                        </span>
                        <span className={`text-sm font-semibold uppercase tracking-wider ${p.color === "blue" ? "text-blue-500" : p.color === "purple" ? "text-purple-500" : "text-cyan-500"}`}>{p.label}</span>
                      </div>
                      <h3 className="text-xl font-bold text-gray-900 mb-2">{p.headline}</h3>
        ...
        <div className="mx-auto max-w-[1320px] px-6">
          <FadeIn>
            <div className="flex items-end justify-between mb-10">
              <div>
                ...
                <p className="mt-2 text-base text-text-secondary max-w-md">
                  From deployment to ongoing optimization — expert services at every stage.
                </p>
              </div>
              <Link
                href="/services"
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {servicesData.map((svc, i) => {
              const SvcIcon = serviceIcons[i] || IconSupport;
              return (
                <FadeIn key={svc.slug} delay={i * 80}>
                  <Link
                    href={`/services/${svc.slug}`}
                    className="group flex flex-col items-center text-center p-6 rounded-xl bg-white border border-light-200 hover:border-ale-200 hover:shadow-md transition-all h-full"
                  >
                    <div className="w-12 h-12 rounded-xl bg-ale-50 flex items-center justify-center mb-4 group-hover:bg-ale-100 transition-colors">
                      <SvcIcon className="w-6 h-6 text-ale" />
                    </div>
          ...
          <div className="sm:hidden text-center mt-6">
            <Link href="/services" className="inline-flex items-center gap-1.5 text-sm font-semibold text-ale">
              All services
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
              </svg>
          ...
          <FadeIn>
            <div className="flex items-end justify-between mb-10">
              <div>
                ...
                <p className="mt-2 text-base text-text-secondary max-w-md">
                  See how organizations worldwide transform with ALE technology.
                </p>
              </div>
              <Link
                ...
                >
                  <div className="p-6 flex-1">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-[10px] text-text-muted">{cs.country}</span>
                    </div>
                    ...
                    {cs.results.length > 0 && (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-text-secondary line-clamp-1">{cs.results[0]}</span>
                      </div>
                    )}
                  </div>
                  <div className="px-6 py-3 border-t border-light-100 bg-light-50/50 flex items-center justify-between">
                    <div className="flex flex-wrap gap-1">
                      {cs.products.slice(0, 2).map((prod) => (
                        <span key={prod} className="text-[10px] text-text-muted">{prod}</span>
                      ))}
                      {cs.products.length > 2 && (
                        <span className="text-[10px] text-text-muted">+{cs.products.length - 2} more</span>
                      )}
                    </div>
                    <svg className="w-4 h-4 text-text-muted group-hover:text-ale group-hover:translate-x-1 transition-all" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M17 8l4 4m0 0l-4 4m4-4H3" />
